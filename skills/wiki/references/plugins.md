...
| **Calendar** | Right-sidebar calendar with word count, task, and link indicators. Install from community plugins. |
...
| **Dataview** *(optional/legacy)* | Only needed if you're on Obsidian < 1.9.10 or want legacy inline queries. The primary data views now use Bases. |

**Thino is pre-installed**. Enable it in Settings → Community Plugins → toggle on. No download needed.
...
5. *(Optional)* Enable Dataview only if you need legacy inline queries

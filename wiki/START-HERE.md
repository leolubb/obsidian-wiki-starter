...
然後用 Obsidian 打開 `my-wiki` 資料夾（Open Folder as Vault）。
...
| 插件 | 做什麼用 |
|------|----------|
| **Dataview** | 用類 SQL 語法在筆記裡寫即時查詢，例如「列出所有 status: developing 的頁面」。 |

#### 筆記撰寫與模板

| 插件 | 做什麼用 |
|------|----------|
| **Templater** | 新建頁面時自動套模板，填好 frontmatter 骨架。模板放在 `_templates/`。 |
| **QuickAdd** | 一鍵巨集：按一個快捷鍵就建好新頁面、自動命名、套模板、放到正確資料夾。 |
| **Editing Toolbar** | 編輯器上方多一排格式按鈕（粗體、標題、列表、表格等）。 |

#### 圖片與媒體

| 插件 | 做什麼用 |
|------|----------|
| **Image Toolkit** | 點擊圖片可放大檢視、旋轉、翻轉。 |
| **Mousewheel Image Zoom** | 滑鼠滾輪直接縮放圖片大小。 |
| **Media Extended** | 在筆記裡直接嵌入並播放影片（YouTube、本地 mp4）。 |
| **Excalidraw** | 手繪白板工具，可以畫流程圖、概念草圖，檔案存在 vault 裡。 |

#### 導覽與組織

| 插件 | 做什麼用 |
|------|----------|
| **Tag Wrangler** | 右鍵 tag 可以批量改名、合併、刪除。 |
| **Homepage** | 每次開 Obsidian 自動跳到指定頁面（預設是本頁）。 |
| **Notebook Navigator** | 資料夾預覽面板，懸停資料夾就能看到裡面的筆記清單。 |
| **Hover Editor** | 按住 `Ctrl` 懸停 `[[連結]]` 會彈出浮動編輯視窗。 |

#### 同步與版本控制

| 插件 | 做什麼用 |
|------|----------|
| **Obsidian Git** | 每 15 分鐘自動 commit + push。開 Obsidian 時自動 pull 最新版本。 |

#### AI 與快速筆記

| 插件 | 做什麼用 |
|------|----------|
| **Smart Connections** | 語意搜尋：靠「意思相近」找頁面，不只靠關鍵字。 |
| **Thino** | 右側邊欄的快速便條面板。隨手記靈感、待辦。 |

#### 任務管理

| 插件 | 做什麼用 |
|------|----------|
| **Tasks** | 待辦事項管理，支援到期日、完成日，可用查詢聚合所有頁面的待辦。 |

> **最低需求：** 只要開 **Templater** + **Obsidian Git** 就能正常使用。其他按需開啟。
...
為了讓 Obsidian 側邊欄保持乾淨，以下資料夾已透過 `userIgnoreFilters` 隱藏。它們仍然存在於 repo 中，是 Claude Code 整合的基礎設施：
...
**想看到它們？** 編輯 `.obsidian/app.json`，從 `userIgnoreFilters` 陣列中刪除不需要隱藏的項目。
